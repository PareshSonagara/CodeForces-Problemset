...
void solve() {
    ...
    llv vec(n);
    invec(vec, n);
    ll zer=0, one=0, two=0, three=0, five=0;
    f(i,n){
        if(vec[i] == 0 && zer < 3) {
            zer++;
        } else if(vec[i] == 1 && one < 1) {
            one++;
        } else if(vec[i] == 2 && two < 2) {
            two++;
        } else if(vec[i] == 3 && three < 1) {
            three++;
        } else if(vec[i] == 5 && five < 1) {
            five++;
        }
        if(zer==3 && one==1 && two==2 && three==1 && five==1) {
            cout<<i+1<<endl;
            r
        }
